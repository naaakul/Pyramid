...
export function FieldsPopover() {
  ...
  return (
    ...
    "
      ...
      <PopoverContent
        align="end"
        className="w-64 p-2 border-ink-border shadow-ink-100"
      >
        <div className="flex rounded-md border border-ink-border mb-2 overflow-hidden">
          <button
            onClick={() => setView("list")}
            className={`flex-1 flex items-center justify-center gap-1.5 text-sm py-1.5 text-ink-900 ${
              view === "list" &&
              "font-medium bg-ink-bg border-r border-ink-border"
            }`}
          >
            <List size={14} />
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 text-sm py-1.5 text-ink-900 ${
              view === "board" &&
              "font-medium bg-ink-bg border-l border-ink-border"
            }`}
          >
            <LayoutGrid size={14} />
        ...
        <div className="flex flex-col">
          {FIELD_CONFIG.filter(
            ...
            <label
              key={field.key}
              className="flex items-center text-ink-text justify-between px-2 py-1.5 text-sm hover:bg-ink-bg rounded cursor-pointer"
            >
              {field.label}
              <Checkbox
