...
export function AddMembersModal({
  ...
  const {
    ...
    queryKey: ["user-search", debounced, taskId],
    queryFn: () => searchUsersByEmail(debounced, taskId),
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <button className="flex items-center gap-1.5 text-sm text-ink-500 hover:text-ink-700">
          <UserPlus size={14} />
          {assignees.length > 0
            ? assignees.map((a) => a.user.name).join(", ")
            : "Add members"}
        </button>
      </DialogTrigger>
      <DialogContent className="max-w-sm ring-ink-border">
        ...
        {debounced.length >= 3 && (
          <div className="max-h-40 overflow-y-auto -mt-1">
            {isFetching && (
              <div className="text-xs text-ink-400 px-1 py-2">Searching...</div>
            )}
            {!isFetching && (results ?? []).length === 0 && (
              <div className="text-xs text-ink-400 px-1 py-2">
