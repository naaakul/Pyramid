...
export function ViewModal({
  ...
  useEffect(() => {
    if (!isReporter)
      apiFetch(`/tasks/${task.id}/view`, { method: "POST" }).catch(() => {});
  }, [task.id, isReporter]);
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-2">
          {task.assignees.map(({ user }) => {
            const invite = task.invites?.find(
              (i: any) => i.invitedUser.id === user.id,
            );
            const watcher = task.watchers?.find(
              (w: any) => w.userId === user.id,
            );
            const status = watcher?.viewedAt
              ? "seen"
              : invite?.status === "ACCEPTED" || !invite
                ? "joined"
                : "sent";
            return (
              <div
              ...
              >
                ...
                {status === "joined" && (
                  <CheckCheck size={14} className="text-ink-400" />
                )}
                {status === "seen" && (
