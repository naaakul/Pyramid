...
export const COLUMN_DEFS: ColumnDef[] = [
  ...
  },
  {
    ...
    label: "Members",
    width: "100px",
    render: (t) => <MembersCell assignees={t.assignees} />,
  },
  {
    key: "dueDate",
    label: "Due Date",
    width: "130px",
    render: (t) => (
      <span className="text-gray-600 text-sm">{formatDate(t.dueDateEnd)}</span>
    ),
  },
  {
    key: "labels",
      ...
      t.labels.length === 0 ? (
        ...
        <div className="flex flex-wrap gap-1">
          {t.labels.map(({ label }) => (
            <span
              key={label.id}
              className="flex items-center gap-1 text-[11px] bg-gray-100 text-gray-600 rounded-full px-2 py-0.5"
            >
